...
export default function Page() {
      ...
      <div className="container py-8 px-4 md:px-6">
        <Card>
          <CardContent className="p-6">
            <p className="text-center text-muted-foreground">No standings data available</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  const contestStanding = data.standing[0];
  ...
  const contestInfo = 
    ...
    contestStanding.contest_info || contestStanding.ContestInfo;
  ...
  if (!contestInfo || !problems || !rankings) {
    return (
      <div className="container py-8 px-4 md:px-6">
        <Card>
          <CardContent className="p-6">
            <p className="text-center text-muted-foreground">Invalid standings data format</p>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="container py-8 px-4 md:px-6 space-y-6">
      ...
      <Card className="shadow-md">
        <CardHeader className="pb-2">
          <CardTitle className="text-2xl font-bold">{contestInfo.title}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">Participants: {contestInfo.participant_count}</span>
            </div>
            <div className="flex items-center gap-2">
              <FileText className="h-4 w-4 text-muted-foreground" />
              <span className="text-sm">Problems: {contestInfo.problem_count}</span>
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b">
                  <th className="sticky left-0 bg-card px-4 py-3 text-left font-medium text-muted-foreground w-[80px]">Rank</th>
                  <th className="sticky left-[80px] bg-card px-4 py-3 text-left font-medium text-muted-foreground w-[160px]">Handle</th>
                  <th className="sticky left-[240px] bg-card px-4 py-3 text-center font-medium text-muted-foreground w-[80px]">Solved</th>
                  <th className="sticky left-[320px] bg-card px-4 py-3 text-center font-medium text-muted-foreground w-[80px]">Penalty</th>
                  {problems.map((problem, index) => {
                    const problemLetter = problem.split('.')[0];
                    return (
                      <th key={index} className="px-4 py-3 text-center font-medium text-muted-foreground min-w-[60px] whitespace-nowrap">
                        {problemLetter}
                      </th>
                    );
                  })}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="sticky left-[240px] bg-inherit px-4 py-3 text-center">
                        <Badge variant={isTopRank ? "default" : "outline"} className="min-w-[28px]">
                          {solvedCount}
                        </Badge>
                      </td>
                      ...
                      {problems.map((problem, problemIndex) => {
                        ...
                        const isAttempted = ranking.attempted.includes(problemLetter);
